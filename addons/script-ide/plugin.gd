...
## Editor setting to hide folder names to decrease the space used by tabs
const HIDE_FOLDER_NAMES: StringName = SCRIPT_IDE + &"hide_folder_names"
...
var hide_folder_names: bool = false
...
func _enter_tree() -> void:
	...
	hide_folder_names = get_setting(HIDE_FOLDER_NAMES, hide_folder_names)
...
func sync_settings():
	...
	for setting in changed_settings:
		...
		elif (setting == HIDE_FOLDER_NAMES):
			# Update to reflect the folder names setting.
			var new_hide_folder_names: bool = get_setting(HIDE_FOLDER_NAMES, hide_folder_names)
			if (new_hide_folder_names != hide_folder_names):
				hide_folder_names = new_hide_folder_names
...
func update_selected_tab():
	...
	if hide_folder_names:
		var file_name: String = scripts_item_list.get_item_text(selected_tab).get_slice("/", 1)
		scripts_tab_container.set_tab_title(selected_tab, file_name)
...
func update_tabs():
	for index in scripts_tab_container.get_tab_count():
		if hide_folder_names:
			var file_name: String = scripts_item_list.get_item_text(index).get_slice("/", 1)
			scripts_tab_container.set_tab_title(index, file_name)
